package com.rainbowsea.tidesound.search.config;
...
public class RedissonAutoConfiguration {
    ...
    // 自动识别导入，我们在 application.yaml 当中配置的 Redis 的配置信息
    @Autowired
    ...
    /**
     * 定义一个 BloomFilter 的Bean对象
     */
    @Bean
    public RBloomFilter rBloomFilter(RedissonClient redissonClient) {


        // 1.如果在 Redis中没有这个key,那么会创建一个key,并且将这个key对应的布隆过滤器对象返回 反之 直接将已经创建好的布隆过滤器返回给你。
