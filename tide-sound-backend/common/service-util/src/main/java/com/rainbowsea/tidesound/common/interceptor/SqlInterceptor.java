package com.rainbowsea.tidesound.common.interceptor;
...
/**
 * 为项目添加上一个自定义的SQL拦截查询自动赋值器，可以查询SQL执行时间
 * Description:
 */
...
public class SqlInterceptor implements Interceptor {
    ...
    private String generateSql(Invocation invocation) {
        ...
        // MappedStatement 代表一条sql语句的映射 MappedStatement都包含了一个唯一的id，一个SQL语句，一个结果映射配置以及一个缓存配置。
        MappedStatement statement = (MappedStatement) invocation.getArgs()[0];
        Object parameter = null;
        if (invocation.getArgs().length > 1) {
            parameter = invocation.getArgs()[1];  //获取sql语句的参数值
        }
        Configuration configuration = statement.getConfiguration();
        BoundSql boundSql = statement.getBoundSql(parameter); // 获取一条sql语句
